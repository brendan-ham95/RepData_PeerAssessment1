...
## 1. Load the data
# Create a data directory if it does not already exist, and unzip the data into that directory.
```{r, echo = TRUE}
...
# Use the aggregate() function to sum the steps for each date. The result will be stored in an object called tSteps
```{r, echo = TRUE}
...
# Use the hist() function to accomplish this task
```{r, echo = TRUE}
...
### taken per day.
# Call the functions mean() and median() on the steps column of our tSteps object.
```{r, echo = TRUE}
...
tMedian
```
# Of the total number of steps taken per day: the mean is 10766.19, and  the median is 10765.

## What is the average daily activity pattern?
### 1. Make a time series plot of the 5-minute interval and the average number of steps taken, averaged across all days.
# We want the daily average amount of steps for each interval. Calculate this by grouping the activity data set by interval and calculating the mean for each interval (after removing the NAs)
```{r, echo = TRUE}
aInt <- activity %>% 
        filter(!is.na(steps)) %>%
        group_by(interval) %>%
...
# Sort our previous data set by decreasing and select the first row.
```{r, echo = TRUE}
...
topInt
```
# The interval across all days that contains the average maximum number of steps (~ 206 steps) is interval 835.

## Imputing missing values
### 1. Calculate and report the total number of missing values in the dataset.
# Sum the TRUE values returned by is.na() on the steps variable
```{r, echo = TRUE}
...
# Imput the missing values using the average daily value for the 5-minute interval where the NA is located.
...
### Do these values differ from the estimate from the first part of the assignment? What is the impact of imputing missing data on the estimates of the total daily number of steps?
```{r, echo = TRUE}
...
# We first have to change all of the dates to a proper date class. Then we can find the specific days using the weekdays() function and appropriately assign "weekday" (Monday - Friday) and "weekend" (Saturday & Sunday).
```{r, echo = TRUE}
dAct <- repAct 
...
### or weekend days.
# Create 2 new datasets. The first will contain the data for weekdays and the second will contain data for weekend days.
```{r, echo = TRUE}
aDay <- dAct %>%
...
# Recombine these two new datasets into one.
```{r, echo = TRUE}
...
# In order to make our panel plot.
```{r, echo = TRUE}
par(mfrow = c(2,1))
par(mar = c(4, 5, 4, 5))
